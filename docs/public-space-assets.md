...
## Life panorama integration, 2026-09-22

The NASA source now has a dedicated offline Life branch in `scripts/render-world-panorama.py` via `--nasa-interior`, implemented by `scripts/integrate-nasa-interior.py`. It accepts only the verified FBX hash above and a refined spaceship descriptor; proxy and measured-terrain modes are rejected. Browser exploration still uses the existing lightweight proxy. The new source is not combined with the old procedural cockpit.

The integration retains the 265 meshes and 474,527 triangles, original UVs, and 106 successfully loaded material images. It applies the existing repair-only material corrections, including the source monitor maps, without the optional authored roughness remap. The source observer `[0, 98, -20.5]` is mapped to the Life Blender observer `[0, 9.95, 1.68]` with a +90-degree X rotation and artistic scale `0.22`; this is an enlarged artistic cabin, **not a calibrated physical dimension**. Four authored area lights illuminate Cupola and Node3. The seven covers and `Cupola_Int_Glass` remain hidden for an independent sky layer. Six-axis camera clearance and seven named window ray samples are checked against visible geometry.

A800 job `168371` completed in 12m21s with exit `0:0` on `d1n41a21g02`. Its isolated directory is `.render-work/nasa-life-20260922-r3/`; output filenames retain the `nasa-r1` suffix, so the directory and job ID identify this revision. It produced three 4096×2048 RGBA panoramas (night/clear/dusk), two separate diagnostic skies, and 27 native 1600×1100 views. All 32 PNGs and four evidence JSON files passed output hash checks. Native clear/dusk perspective views include a diagnostic HDRI background, while all three panoramas preserve transparency. Seven specified window samples have alpha zero in every phase; this is a sample check, not proof that no unintended opening exists elsewhere.

The first two integration jobs, `168355` and `168368`, failed after 15 seconds because Blender's lazy image decoding made early `has_data` checks report missing textures. Reading size/pixels before checking resolved that issue; failed directories remain intact. The successful r3 script snapshots match the current integration/render/validation source hashes. All three jobs have ended; unrelated training jobs `167803` and `168319` were untouched.

Independent visual review of nine contact sheets and three full-resolution native images found **photoreal FAIL**: Cupola is too dark relative to Node3, the rear top has an unexplained rectangular black region, and close rims and broad walls retain segmented or flat shading. See [the complete review](review-20260922-nasa-integration.md). Consecutive clean rounds remain **0**. Technical integration and the user's source preference do not count as visual approval.

An isolated localhost preview substitutes only the spaceship/night catalog entry, using 2048/4096 WebP derivatives. Run `python scripts/serve-panorama-review.py --manifest .render-work/nasa-life-20260922-r3/preview/catalog.json --port 8766`, then open `http://127.0.0.1:8766/life.html` and select the ship. This catalog and its textures are ignored review artifacts, not a production installation. HTTP checks passed; browser control authentication prevented an actual runtime sky/constellation interaction review. No runtime script, Home content, source high-poly download, or production panorama manifest was changed. The production manifest remains SHA-256 `8ecb6a5fc4990027c9fb8a1a91888e4a14478fa569a6cb14b1ec82a94633d7b0`.
