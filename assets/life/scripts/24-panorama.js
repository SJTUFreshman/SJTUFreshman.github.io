(() => {
  ...
  const api = window.NightPanorama = {
    ...
    getSnapshot() { return { ready: api.ready, status, quality, tier, production, scene: active, skyMode, error }; },
    setMode(value) { if (!['observe', 'explore', 'sky'].includes(value)) return false; mode = value; visibility(); return true; },
    setSkyMode(value) { if (!['night', 'clear', 'dusk'].includes(value)) return false; skyMode = value; return load(); },
    setQuality(value) { if (!['auto', ...tierOrder].includes(value)) return false; quality = value; requestKey = ''; return load(); },
    ...
    render(next) {
      ...
      if (mode !== 'observe' || !pose.visible) return;
    ...
    coversSky() { return mode === 'observe' && pose.visible && Boolean(layers.get('sky')?.texture); },
    canSeeSky(horizontal, vertical) {
      const layer = layers.get('scene');
      if (mode !== 'observe' || !layer?.alpha || !layer.texture) return true;
    ...
    dispose() { controller?.abort(); requestToken++; for (const layer of layers.values()) disposeLayer(layer); layers.clear(); }
  };
  ...
  async function load() {
    ...
    try {
      ...
      if (token !== requestToken) return false;
      tier = foreground.tier; status = 'prerendered'; visibility(); notify(); return true;
    } catch (failure) {
  ...
  function ensureLayer(name) {
    ...
    const shaders = [compile(context, context.VERTEX_SHADER, vertexSource), compile(context, context.FRAGMENT_SHADER, fragmentSource)];
    ...
    canvas.addEventListener('webglcontextlost', event => { event.preventDefault(); layer.lost = true; layer.texture = null; layer.alpha = null; controller?.abort(); requestToken++; requestKey = ''; status = 'error'; error = 'Panorama graphics were interrupted.'; visibility(); notify(); }, { once: true });
    canvas.addEventListener('webglcontextrestored', () => { if (layers.get(name) === layer) layers.delete(name); requestKey = ''; load(); }, { once: true });
    return layer;
  }
  ...
  async function loadLayer(name, variant, token, signal) {
    ...
    const preferred = quality === 'auto' ? (window.matchMedia('(pointer: coarse)').matches || navigator.connection?.saveData ? 'low' : 'medium') : quality;
  ...
  function draw(layer, orientation) {
    const context = layer.context, budget = quality === 'high' ? 8294400 : quality === 'low' ? 1228800 : 3686400;
  ...
  function visibility() { for (const layer of layers.values()) layer.canvas.style.visibility = mode === 'observe' && pose.visible && layer.texture && !layer.lost ? 'visible' : 'hidden'; }
