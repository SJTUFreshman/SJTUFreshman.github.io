...
(function () {
    ...
    const SCENES = ['transit', 'lakeshore', 'observatory', 'spaceship', 'train', 'room', 'loop'];
    const COPY = {
        en: {
            where: 'YOU ARE HERE', choose: 'Choose a place', title: 'Somewhere under the stars',
            intro: 'Seven places to wander. Stay as long as you like.', close: 'Close places',
            explore: 'Explore', sky: 'Just the sky', view: 'View mode', current: 'Here now',
            ...
            distance: 'wandered', pilot: 'AT THE HELM', pilotHint: 'W / S · throttle   A / D · turn   Q / E · roll\nSpace · brake   F · leave the seat',
            touchPilotHint: '↑ / ↓ · throttle   ← / → · turn   Drag · look',
            pilotForward: 'Increase throttle', pilotBack: 'Decrease throttle', pilotLeft: 'Turn left', pilotRight: 'Turn right',
            move: 'Move through the environment', forward: 'Walk forward', back: 'Walk backward', left: 'Move left', right: 'Move right',
            entered: 'Now in', switchMode: 'Switch between walking and observing the sky',
            ...
            spaceship: ['The drifting ship', 'Take the helm, or watch from the cabin.', 'ORBIT / ENGINE HUM'],
        ...
        'zh-CN': {
            ...
            intro: '七处可以闲逛的地方。去走走，也可以什么都不做。', close: '关闭场景选择',
            explore: '自由漫游', sky: '只看星空', view: '视角模式', current: '正在这里',
            ...
            distance: '漫游', pilot: '驾驶中', pilotHint: 'W / S · 油门   A / D · 转向   Q / E · 翻滚\n空格 · 制动   F · 离开驾驶位',
            touchPilotHint: '↑ / ↓ · 油门   ← / → · 转向   滑动 · 环顾',
            pilotForward: '增加油门', pilotBack: '减小油门', pilotLeft: '向左转向', pilotRight: '向右转向',
            move: '在场景中移动', forward: '向前走', back: '向后走', left: '向左移动', right: '向右移动',
            entered: '已来到', switchMode: '切换自由漫游和星空观测',
            transit: ['永夜中转地', '空旷原野里，一堆未尽的篝火。', '余烬 / 寂静'],
            lakeshore: ['静夜湖岸', '栈桥延向湖心，远处灯塔明灭。', '湖水 / 倒影'],
            observatory: ['旧日天文台', '越过石阶，在旧穹顶下抬头。', '石阶 / 星光'],
            spaceship: ['漂流飞船', '握住操纵杆，或在生活舱里散步。', '轨道 / 低鸣'],
        ...
        'zh-TW': {
            ...
            intro: '七處可以閒逛的地方。去走走，也可以什麼都不做。', close: '關閉場景選擇',
            explore: '自由漫遊', sky: '只看星空', view: '視角模式', current: '正在這裡',
            ...
            distance: '漫遊', pilot: '駕駛中', pilotHint: 'W / S · 油門   A / D · 轉向   Q / E · 翻滾\n空白鍵 · 制動   F · 離開駕駛位',
            touchPilotHint: '↑ / ↓ · 油門   ← / → · 轉向   滑動 · 環顧',
            pilotForward: '增加油門', pilotBack: '減小油門', pilotLeft: '向左轉向', pilotRight: '向右轉向',
            move: '在場景中移動', forward: '向前走', back: '向後走', left: '向左移動', right: '向右移動',
            entered: '已來到', switchMode: '切換自由漫遊和星空觀測',
            transit: ['永夜中轉地', '空曠原野裡，一堆未盡的篝火。', '餘燼 / 寂靜'],
            lakeshore: ['靜夜湖岸', '棧橋延向湖心，遠處燈塔明滅。', '湖水 / 倒影'],
            observatory: ['舊日天文台', '越過石階，在舊穹頂下抬頭。', '石階 / 星光'],
            spaceship: ['漂流飛船', '握住操縱桿，或在生活艙裡散步。', '軌道 / 低鳴'],
    ...
    function boot() {
        ...
        root.innerHTML = '<div class="world-dock">' +
            ...
            '<footer class="world-menu-footer"><div class="world-view-control" role="group" id="worldViewControl"><button type="button" data-world-mode="explore" aria-pressed="true" data-world-copy="explore"></button><button type="button" data-world-mode="sky" aria-pressed="false" data-world-copy="sky"></button></div><div class="world-menu-tools"><button class="world-sound-button" id="worldSound" type="button" aria-pressed="false">' + icon('sound') + '<span></span></button><button class="world-icon-button" id="worldReset" type="button">' + icon('reset') + '</button></div></footer>' +
        ...
        let api, lang = '', t, snapshot = {}, open = false, suspended = false, mode = 'explore';
        ...
        function language() {
            ...
            pad.querySelectorAll('button').forEach(node => setAttr(node, 'aria-label', t[node.dataset.worldMove]));
        ...
        function updateMotion() {
            ...
            heldMoves.forEach(value => { if (value === 'forward') forward++; if (value === 'back') forward--; if (value === 'left') strafe--; if (value === 'right') strafe++; });
        ...
        function menuState(value, restore = true) {
            ...
            if (open) {
                const active = menu.querySelector('[data-world-id="' + (snapshot.id || 'transit') + '"]');
        ...
        function changeMode(next) {
            mode = next; stopMotion();
            if (api && typeof api.setViewMode === 'function') api.setViewMode(next);
            refresh();
        }
        function refresh() {
            if (!t) return;
            ...
            const id = SCENES.includes(snapshot.id || (api && api.currentId)) ? (snapshot.id || api.currentId) : 'transit';
            const currentMode = snapshot.mode || (api && api.mode);
            if (currentMode === 'explore' || currentMode === 'sky') mode = currentMode;
            ...
            root.querySelectorAll('[data-world-mode]').forEach(node => setAttr(node, 'aria-pressed', node.dataset.worldMode === mode));
            ...
            setText(status, loadError ? t.error : ready ? '' : t.loading);
            status.hidden = ready && !loadError;
            ...
            const messageText = ready && mode === 'explore' ? localText(snapshot.message) : '';
            setText(actionMessage, messageText);
            actionMessage.hidden = !messageText;
            const hint = mode === 'sky' ? t.skyHint : snapshot.piloting ? (coarse.matches ? t.touchPilotHint : t.pilotHint) : coarse.matches ? t.touchHint : t.walk + '     ' + t.hint;
            setText(byId('worldWalkHint'), hint);
            pad.querySelectorAll('button').forEach(node => {
            ...
            pad.hidden = !ready || mode !== 'explore' || open;
        ...
        root.querySelectorAll('[data-world-id]').forEach(button => button.addEventListener('click', () => {
            ...
            if (mode !== 'explore') changeMode('explore');
            menuState(false); refresh();
        }));
        root.querySelectorAll('[data-world-mode]').forEach(button => button.addEventListener('click', () => changeMode(button.dataset.worldMode)));
        ...
        pad.querySelectorAll('button').forEach(button => {
            button.addEventListener('pointerdown', event => {
                if (event.button !== 0 || open || suspended || mode !== 'explore') return;
                event.preventDefault(); button.setPointerCapture(event.pointerId);
                heldMoves.set(event.pointerId, button.dataset.worldMove); button.classList.add('is-held'); updateMotion();
            });
            ...
            button.addEventListener('keydown', event => {
                if ((event.key === ' ' || event.key === 'Enter') && !event.repeat) { event.preventDefault(); heldMoves.set('keyboard', button.dataset.worldMove); button.classList.add('is-held'); updateMotion(); }
            });
